...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class LeadTarget2d : FsmStateAction
	{
		...
		void DoLeadTarget ()
		{
			...
			if (Target)
			{
				...
				var velocityPosition1 = Target.transform.position + ((new Vector3(Target.rigidbody2D.velocity.x, Target.rigidbody2D.velocity.y, 0) * mult) * (distTarget/projectileSpeed));
				var velocityDist1 = Vector3.Distance(velocityPosition1, Origin.transform.position);
		
				// second calc., using distance from first calc.
				var velocityPosition2 = Target.transform.position + ((new Vector3(Target.rigidbody2D.velocity.x, Target.rigidbody2D.velocity.y, 0) * mult) * (velocityDist1/projectileSpeed));
				var velocityDist2 = Vector3.Distance(velocityPosition2, Origin.transform.position);
			
				// third calc., using distance from second calc.
				var TargetInterceptPosition = Target.transform.position + ((new Vector3(Target.rigidbody2D.velocity.x, Target.rigidbody2D.velocity.y, 0) * mult) * (velocityDist2/projectileSpeed));
